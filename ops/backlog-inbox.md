...
<task_item>
  ...
  <description>Owner-identified gap: the entire game lives in exactly one scene (scenes/core/Main.tscn) — no interior buildings, no doors, no map/scene transitions exist at all. Spec (docs/research/TASK-352-spec.md) recommends Godot's SceneTree.change_scene_to_file() via a thin SceneLoader autoload + door/warp marker convention, reusing this codebase's existing SignalBus self-registration pattern (GridManager.gd's "SignalBus.grid_manager = self / null" idiom) rather than a single ever-growing mega-scene with toggled visibility. Scope is foundation-only: fix TimeManager (promote to a true autoload — see researcher_notes), SceneLoader.gd, Door.gd/tscn convention, rename Main-&gt;World, and ONE proof-of-concept interior (FarmHouse). Converting other buildings (market, temple, trader stall) into enterable interiors is explicit follow-up work, not in this task.</description>
