...
<task_item>
  ...
  <description>Gap-analysis finding (AI-ENG-001 loop, 2026-09-04): FarmHouse.gd's interior (verified directly in scenes/interiors/FarmHouse.gd) has zero player-facing customization -- the bed (FarmHouseBed), the shrine (FarmHouseShrine), and 4 decoration sprites (water_jar, clay_stove, pha_khao_ma, mohom_cloth from DECOR_PATHS) are all placed at hardcoded tile positions with no interact-to-change path; the player can never choose, buy, or swap any of it. Gemini genre research (HM:BtN, Stardew Valley, Animal Crossing) on house-customization mechanics recommended, for a codebase without an existing grid/collision furniture-placement system, an "anchor slot" model over a full drag-and-drop editor: keep each decor position fixed, but make it an interactable with a `slot_type`, and let the player select any owned item matching that slot from a small list -- the same shape as an inventory pick, not a new placement system. This is purely additive and reuses two patterns already proven in this codebase: GameData's small persistent dict convention (mirrors `tool_tiers`) for `decor_choices: Dictionary = {"shrine": "basic", ...}`, and the Market's existing buy-list UI (just improved 2026-09-04 with the 3-column readability fixes) for purchasing alternate decor skins. Scope: GameData.gd (new `decor_choices` dict + persistence in SaveManager.gd, SAVE_VERSION bump if the schema requires it), FarmHouseShrine.gd/FarmHouse.gd (read `decor_choices` when building each slot's sprite instead of a single hardcoded texture), MarketShop.gd's buy-offer list extended with a couple of alternate-style decor items (e.g. "ornate shrine" alongside the existing "basic" one), and a test file following this repo's existing `_check()` convention. Not yet dispatched -- filed for the next sprint slot. No change to FarmHouseBed/FarmHouseShrine's existing gameplay function (sleep, weather forecast) -- purely a visual-choice layer on top.</description>
