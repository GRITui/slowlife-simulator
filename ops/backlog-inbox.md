...
<task_item>
  ...
  <researcher_notes>Owner decisions locked in via sequential AskUserQuestion verification (not assumed): (1) sequencing — #199 and #203 run in parallel, but EdgeTransition.gd specifically does not merge until #199 lands; (2) Phase-1 slice confirmed as CoastalArea with CarpenterUpgrade added to the move (a relocation assessment was run against every NPC/object position in World.tscn/World.gd — only CarpenterUpgrade (18,8) and Noticeboard (16,9) were close enough to the cut zone to matter; Noticeboard stays in World since a community board belongs in the main hub, not a satellite area); (3) the SAVE_VERSION bump stays in scope for this task rather than being split out, migration test written first per this project's save-schema rule; (4) sprint order across current follow-ups is #199 -&gt; #203 -&gt; #201 -&gt; #200 -&gt; #202. Load-bearing finding surfaced while writing the spec, not assumed: SaveManager.gd currently writes player_pos as a hardcoded literal that's never read back on load, and there's no scene_path field at all — harmless with one scene, silently wrong the moment CoastalArea exists. Not yet started.</researcher_notes>
</task_item>
