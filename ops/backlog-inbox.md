...
<task_item>
  ...
  <status>NEEDS_OWNER_REVIEW</status>
  <priority>N/A (owner scope/direction call, not yet actionable)</priority>
  <title>Cooking has no recipe-discovery/unlock system -- worth adding one?</title>
  <description>Gap-analysis finding (AI-ENG-001 loop, 2026-09-04). Initial premise checked and REJECTED before filing (see researcher_notes): the idea that cooking "needs" a skill/mastery stat like fishing_skill/mining_skill turned out to be wrong -- Gemini's genre research confirmed neither Harvest Moon: Back to Nature nor Stardew Valley has an XP-based cooking skill; this codebase's flat cooking (any of 36 recipes craftable immediately given ingredients + infrastructure, confirmed directly in scripts/interactables/CookingStation.gd, explicitly documented there since TASK-055: "every recipe craftable right now") already matches genre convention on that specific point.

  The real, verified divergence found while checking that premise: both reference games use RECIPE ACQUISITION as their primary cooking progression axis instead of a skill stat -- HM:BtN via TV Shopping/villager friendship/blind experimentation, Stardew via "The Queen of Sauce" TV channel + villager mail at high friendship + other-skill-level rewards. This codebase has no equivalent at all -- every recipe is known from the start; the ONLY gate is `requires_infrastructure` (coarse, unlocks many recipes at once) + season + raw ingredients on hand, confirmed via `scripts/resource_types/RecipeData.gd` and `scripts/interactables/CookingStation.gd`. Whether to add an incremental discovery/unlock layer (e.g., recipes tied to NPC affinity milestones, mirroring this codebase's own gift-affinity system already used elsewhere) is a genuine scope/direction call, not a bounded fix -- it touches the core "which recipes exist for the player" model across all 36 entries and would need a new `known_recipes`-style persistent field, UI to show locked-vs-known, and a real decision about which recipes attach to which unlock triggers (NPC affinity? festival completion? purchase?). Question for the owner: pursue a recipe-discovery/unlock layer as a real task (and if so, roughly what unlock triggers fit this game's existing systems best), or leave cooking's current flat-availability model as-is?</description>
