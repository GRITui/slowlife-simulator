...
<task_item>
  ...
  <description>Owner request (2026-09-04), filed immediately after TASK-376 landed the basic portrait display. Currently each PORTRAIT_PATHS speaker has exactly ONE static portrait image shown for every line of dialogue, regardless of context (a gift being well-received vs. poorly-received, a rival's confession-dilemma escalation, a festival greeting vs. a routine daily line, etc.). Scope (needs design/scoping pass before implementation, not yet locked): (1) decide which speakers/situations actually warrant a mood variant -- likely start with the 6 PORTRAIT_PATHS speakers plus whichever romance-candidate/rival NPCs get portraits once TASK-373 lands, not a blanket requirement for every NPC; (2) decide the mood taxonomy (e.g. neutral/happy/sad or neutral/pleased/annoyed -- keep it small, this project's "cozy, non-punishing" design philosophy argues against an elaborate emotion system); (3) decide the trigger mechanism -- e.g. extend SignalBus.show_dialogue's signature (or add a new signal) to optionally carry a mood/reaction tag that World.gd's _on_show_dialogue uses to pick among per-speaker portrait variants (PORTRAIT_PATHS becoming Dictionary[String, Dictionary] keyed by speaker then mood, defaulting to "neutral" when unspecified so every existing call site keeps working unchanged); (4) source the actual mood-variant art -- likely another Draw Things batch per speaker, following tonight's established prompt-writing convention (natural-language paragraphs, reuse the winning style/seed approach from the 6 portraits that just landed).</description>
  <researcher_notes>Deliberately filed as SPECCED-but-not-locked rather than a fully bounded task -- the taxonomy/trigger-mechanism decisions in points 2-3 are real design calls, not something to lock down without owner input first, same discipline as TASK-361/374's phased-scoping approach. Re-scope precisely (or split into a design-decision pass + implementation task) before dispatching.</researcher_notes>
</task_item>
...
<task_item>
  ...
  <description>Found while Code-Quality-Reviewing TASK-379's own regression test. `scripts/narrative/ScheduleDB.gd`'s SCHEDULES dict has real, authored entries for "fah" and "ek" (multiple time-windowed waypoints, with flavor comments like "# paddy rivalry", "# canal mornings"), but both use `scenes/entities/RomanceNPC.gd` as their backing script, and RomanceNPC.gd's `_ready()` never reads ScheduleDB at all (confirmed directly -- no ScheduleDB reference anywhere in that file). Only `VillagerNPC.gd`-backed NPCs (headman, vet, elder, child, handler, nok) actually apply their SCHEDULES entry via `global_position = _schedule_pos` in `_ready()`. This means fah and ek have always been static (fixed at their scene position) despite having real, apparently-intentional daily-movement data authored for them -- likely dormant/unnoticed because neither was ever instanced before TASK-373. Scope (needs an owner call before implementation, not locked): decide whether fah/ek SHOULD move around per their existing schedule data (in which case, port VillagerNPC.gd's schedule-application logic into RomanceNPC.gd, or extract it into a shared mixin/helper both scripts call) or whether static positioning for romance candidates is actually the right call for this project (in which case, delete the now-confirmed-dead SCHEDULES entries for fah/ek rather than leave misleading data sitting in the file).</description>
  <researcher_notes>Test-schedules.gd's own pre-existing assertion ("fah placed at schedule waypoint (boot 06:00)") had been asserting behavior that was never actually true for fah's real script -- fixed to check reality (a stable static position) as part of landing TASK-379, since the old assertion was simply wrong, not a meaningful regression guard. This task is about the underlying feature-gap this exposed, not the test fix itself (already done).</researcher_notes>
</task_item>
