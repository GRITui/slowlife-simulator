extends Node2D
## MountainCaveSpot — TASK-337. A secondary, unlockable mining spot past the
## ridge, gated behind GameData.mining_skill reaching its cap (3). Reuses
## data/ore/ore.json's existing 3 ores verbatim (no new items); the "richer
## vein, harder to reach" framing lives in the rarity weights, inverted from
## MiningSpot.gd so the rare ore (silver_ore) is the MOST likely result here
## instead of the least. Cave digs do NOT bump mining_skill (the gate already
## held) and do NOT re-trigger the deep_miner milestone — MiningSpot owns
## that.

const ORE_PATH: String = "res://data/ore/ore.json"
## Stamina cost per dig attempt (same as MiningSpot — same activity, harder
## to reach, not harder on the body).
@export var dig_cost_stamina: float = 8.0
## Skill cap (matches 3-entry ore roster; cave draws from the same pool).
@export var skill_cap: int = 3
## Proximity radius (matches SluiceGate/CarpenterUpgrade/MiningSpot).
@export var interact_radius: float = 56.0

var spot_name: String = "Mountain Cave"
var _player_in_range: bool = false
var _roster: Array = []
var _area: Area2D = null

func _ready() -> void:
	add_to_group("mountain_cave_spot")
...
func _build_interact_area() -> void:
	...
	collider.debug_color = Color(0.4, 0.3, 0.6, 0.32)
...
## Roll an ore by rarity weight — inverted from MiningSpot.gd:
## common 1.2 / uncommon 2.5 / rare 4.0. Same 3 ores, but silver (rare) is
## now the MOST likely result, framing this as a richer vein.
...
func dig() -> bool:
	...
	# Deliberately does NOT bump mining_skill and does NOT re-trigger the
	# deep_miner milestone — MiningSpot.gd owns that surface (the player
	# already earned the cap to get here). This spot's ore find is
	# independent of that progression milestone.
	SignalBus.craft_completed.emit(item, 1) # reuse item-gained signal (no new orphan)
	SignalBus.show_dialogue.emit(spot_name, "From the cave mouth past the ridge: %s! (+%d harmony)" % [
