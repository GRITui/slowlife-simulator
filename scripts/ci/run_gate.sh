...
run_family_marriage_reactions() {
	# TASK-384 one-shot family marriage reactions (5x FlavorNPC + Somchai
	# via VillagerNPC). Covers both code paths: normal lines before
	# marriage, exact locked line on next talk, revert on second talk,
	# wrong-candidate unaffected, and save/load round-trip.
	echo "== family-marriage-reactions gate: tests/test_family_marriage_reactions.gd =="
	godot --headless --path . --script res://tests/test_family_marriage_reactions.gd
}

run_family_gift_hints() {
	# TASK-385: family NPC once-per-day gift hint for their linked
	# candidate (first talk only, no affinity reward, Somchai/Kwan
	# deliberately excluded) + family_gift_hint_last_day save round-trip.
	echo "== family-gift-hints gate: tests/test_family_gift_hints.gd =="
	godot --headless --path . --script res://tests/test_family_gift_hints.gd
}

run_npc_birthdays() {
	# TASK-388: NPC birthdays + birthday-gift affinity bonus for the 6
	# romance candidates (static NPC_BIRTHDAYS table, 2x gift delta on
	# the exact (season, day) triple, birthday line, RelationshipStatus
	# BirthdayLabel discovery path). No persisted state by design.
	echo "== npc-birthdays gate: tests/test_npc_birthdays.gd =="
