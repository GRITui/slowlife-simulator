...
func _initialize() -> void:
	...
	gd.add_affinity("niran", 100)
	niran.try_interact() # proposal path
	_check(gd.married and gd.spouse == "niran", "proposal accepted")
	...
	for d in _dialogue_hits:
		if d[0] == "Niran" and String(d[1]).contains("on the way"):
			saw_pregnant = true
	...
	for d in _dialogue_hits:
		if d[0] == "Niran" and String(d[1]).contains("baby is here"):
			saw_birth = true
	...
	for d in _dialogue_hits:
		if d[0] == "Niran" and String(d[1]).contains("walking now"):
			saw_toddler = true
	...
	for d in _dialogue_hits:
		if d[0] == "Niran" and String(d[1]).contains("Happy anniversary"):
	...
	var ok: bool = niran.try_interact()
	...
	gd.inventory.clear()
	gd.affinity["niran"] = 0
	gd.add_affinity("niran", 70) # close tier (60-89)
	_dialogue_hits.clear()
	var rival_hits: int = 0
	for i: int in 10:
		niran.try_interact() # no gift/krathong held -> falls through to _talk()
